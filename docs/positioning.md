...
- 自建 = 完整能力,开源不阉割
- 不做多文件站点托管、不做构建
- **所有链接都有有效期,没有永久链接**——匿名最长 3 天(一次性、不可续)、登录最长 30 天/期,到期前可续期,弃置即自动过期清场。这不是限制而是身份:"稿"是阶段性的,不是要长期挂着的站点(守"不做托管"红线,也是隐私与滥用的天然泄压阀)。
- **匿名故意保持轻量,鼓励登录**——匿名只保"零注册 30 秒发一稿"的顺滑首体验(单页 ≤512KB / 3 天 / 合集 ≤3 篇);每人一链、版本历史/回滚、续期、更大体积与合集都是登录(免费)才解锁。重度/认真使用自然导向账号,但绝不给头牌"发一稿"加注册门槛。
