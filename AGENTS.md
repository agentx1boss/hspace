...
- **没有永久链接**(产品级不变量):所有链接都有有效期,匿名 7 天(一次性、不可续)/ 登录 30 天/期,到期前可续、弃置即自动过期。API 与插件都产生不了永久页;`expiresIn:null` 只当"续到档内上限"。唯一例外是**第一方置顶内容**(promo 册子/落地演示),靠直接改库 `expires_at=NULL` 手动置顶,不是产品能力。改动别退回"永久"叙事(见 positioning §8)。
...
- `#p=` fragment 命名空间已被一键链接占用(密码页只预填、不自动提交);不要拿 `#p` 表示 page 或其他锚点。
