...
const MATRIX: Record<Lang, MxRow[]> = {
  en: [
    ...
    { label: "Link lifetime", anon: "7 days · one-shot", user: "up to 30 days" },
    { label: "Renew before expiry", anon: "—", user: "✓" },
    { label: "File size", anon: "512 KB", user: "2 MB" },
    { label: "Docs per collection", anon: "up to 5", user: "up to 50" },
  ...
  zh: [
    ...
    { label: "链接有效期", anon: "7 天 · 一次性", user: "最长 30 天" },
    { label: "到期前续期", anon: "—", user: "✓" },
    { label: "文件体积", anon: "512 KB", user: "2 MB" },
    { label: "合集篇数", anon: "最多 5", user: "最多 50" },
...
const L: Record<Lang, Record<string, string>> = {
  en: {
    htmllang: "en",
    title: "HSpace — Ship your AI-written docs to exactly the right people",
    desc: "Publish Markdown privately: a clean reading page behind a password — read receipts, revocable access, links that expire. Private sharing for Cursor / Claude Code developers.",
    ogTitle: "HSpace — Ship to one, not to all.",
    ogDesc: "Publish Markdown privately: a clean reading page behind a password — read receipts, revocable access, links that expire.",
    ...
    heroTag: "Built for AI coding · Private sharing, not a public link",
    heroH1: `Ship <span class="hl">to one</span>, not to all.`,
    heroLead: "Share what AI writes — without losing control. HSpace turns .md into a clean, password-gated reading page (HTML runs as-is too): read receipts, revocable access, links that expire.",
    ...
    s2sub: "Nothing to install — they open the link to a clean reading page: sidebar table of contents, cross-doc nav, light/dark themes. HTML pages run as-is.",
    ...
    tMetric: "Metric",
    diffH: "Not another public share link",
    diffSub: "Notion links, Gists, HackMD — public by default, and none of them built for reading a long draft. Sending work to teammates and clients needs a different toolkit.",
    anti: "Not a: file drive · collab editor · notes app · site builder · public gallery.",
    ...
    c2t: "🎯 Sent wrong? Take it back", c2b: "Per-recipient links mean kicking one person out is instant and doesn't change anyone else's password; changing the shared password shuts the door on new visits. Receipts tell you it was opened — and with per-recipient links, by whom.",
    ...
    trySub: "Below is a real password-gated HSpace collection — Markdown docs rendered as reading pages, plus one interactive HTML page. Enter the password and you’ve just been a recipient.",
    ...
    st1t: "Open a file or folder", st1b: "Any .md / .html file, or right-click a folder to publish a collection.",
    ...
    featSub: "Your docs are born in your editor and AI chats — sharing should happen there too.",
    vsFile: "product-spec.md",
    f1t: "Markdown, beautifully published", f1b: "Publish .md and it renders into a reading page built for actually reading: headings, tables, code blocks, a sidebar table of contents, light/dark themes and reader preferences.",
    f2t: "Document collections", f2b: "Bundle a batch of md/html into one link, one password, one table of contents with cross-doc nav in the sidebar.",
    f3t: "Per-recipient links", f3b: "Signed in, give each recipient their own password: know who opened it and how often; kick one out instantly, without changing everyone else's.",
    ...
    faqH: "You might ask",
    faqQ1: "Will it get indexed by search engines?", faqA1: "No — every shared page is noindex. Publishing from the extension, the MCP server or the Claude Code plugin always generates a password, so a forwarded link is just a wall without it. (Calling the raw API, you can deliberately omit the password to publish a public page.)",
    ...
    faqQ3: "Sent to the wrong person / want to cut access?", faqA3: "With per-recipient links, revoking one person takes effect immediately and leaves everyone else alone. Otherwise: change the shared password to shut the door on new visits, or delete the page and the link goes dark right away. One caveat — a reader who already entered the shared password keeps access through their session cookie for up to 24h, so delete the page if you need them out this second.",
    faqQ4: "Is it free? Any limits?", faqA4: "Core capabilities are free right now — password sharing, collections, receipts, per-recipient links, versioning, all un-gated. Anonymous & instant, no signup: single file ≤ 512KB, up to 7 days, ≤ 5 docs per collection, 50/day. Sign in (free) to unlock more — up to 30-day renewable links, 2MB, per-recipient links, version history, bigger collections. Phishing and malicious content are prohibited and taken down; core stays free and open source.",
    ...
    ctaBandH: "Next doc your AI writes, try sharing it this way",
  ...
  zh: {
    ...
    desc: "AI 写的稿，排成私密阅读页：密码链接、可撤回、会过期、有访问回执。为 Cursor / Claude Code 开发者而生的私密 Markdown 分享。",
    ogTitle: "HSpace — 稿出即递,点开即读,心里有数",
    ogDesc: "AI 写的稿，排成私密阅读页：密码链接、可撤回、会过期、有访问回执。",
    navHow: "如何使用", navFeat: "功能", navCompare: "对比", navFaq: "FAQ", navConsole: "控制台", install: "安装插件",
    refBanner: "有人通过 HSpace 把一个页面分享给了你——链接 + 密码,只给该看的人。",
    refCta: "你也发一个 →",
    heroTag: "为 AI 编程而生 · 私密分享,不是公开链接",
    heroH1: `<span class="hl">稿出即递</span>,点开即读,心里有数`,
    heroLead: "AI 写的稿，发出去也不失控：密码链接、可撤回、会过期、有访问回执。HSpace 把 .md 排成只给该看的人打开的阅读页(HTML 原样能跑)。",
    ...
    s2h: "对方那边:输一次密码,通览全部",
    s2sub: "对方什么都不用装,点开就是排好版的阅读页:侧栏目录、篇间导航、亮暗主题。HTML 稿原样能跑。",
    tocTitle: "Q3 增长方案", tocMeta: "5 篇 · 2026-07 分享",
    d1: "总览与目标", d2: "数据分析", d3: "渠道策略", d4: "预算与排期", d5: "附录:竞品对比",
    capToc: "合集目录页", capRead: "Markdown 篇目 · 侧栏导航",
    tMetric: "指标",
    diffH: "不是又一个公开分享链接",
    diffSub: "Notion 分享链接、Gist、HackMD 都默认公开,也都不是为「认真读完一份长稿」设计的;给同事和客户看的东西,要的是另一套能力。",
    anti: "我们不是:网盘 · 协作工具 · 笔记应用 · 建站平台 · 公开画廊。",
    c1t: "🚀 30 秒发出去", c1b: "零注册、零配置,编辑器或 AI 对话里一键发布,链接+密码一次粘贴。别家的密码保护要付费套餐加一堆配置,这里是默认。",
    c2t: "🎯 发错了?收得回", c2b: "每人一链,踢掉一个人即时生效,不用换所有人的密码;改共享密码挡住后续访问。打开了几次有回执,每人一链时还知道是谁。",
    c3t: "🔁 链接是活的", c3b: "稿在改,链接不变——review 意见改完直接更新,历史版本可回滚,不用再发「最终版_v3」。",
    tryH: "亲自体验一次",
    trySub: "下面是一本真实的 HSpace 私密合集——Markdown 篇目渲染成阅读页,另有一篇交互 HTML。输入密码,你就完成了一次接收方的完整体验。",
    tryTh: `一份 3 篇的搭建教程合集 · 密码 <b>1024</b>`, trySlug: "omcenj1",
    howH: "三步发走", howSub: "在编辑器里,或直接在 AI 对话里。",
    st1t: "打开文件或文件夹", st1b: "任意 .md / .html 文件,或右键一个文件夹发布为合集。",
    st2t: "一键发布", st2b: "点云图标,自动生成 4 位密码,内容上传到边缘。",
    st3t: "链接 + 密码发走", st3b: "两者一起进剪贴板,微信 / 邮件里一次粘贴,交给该看的人。",
    featH: "为 AI 编程工作流而生",
    featSub: "稿诞生在编辑器和 AI 对话里,分享也应该在那里发生。",
    vsFile: "产品方案.md",
    f1t: "Markdown 成刊", f1b: "发布 .md 自动渲染成真能读下去的阅读页:标题、表格、代码块、侧栏目录,亮暗双主题与阅读偏好。",
    f2t: "文档合集", f2b: "一批 md/html 打包成一个链接、一个密码、一个目录页,侧栏里篇间自由导航。",
    f3t: "每人一链", f3b: "登录后给每个接收者一个专属密码:谁看了、看了几次一清二楚;踢掉一个人即时生效,不用换所有人的密码。",
    ...
    faqH: "你可能想问",
    faqQ1: "内容会被搜索引擎收录吗?", faqA1: "不会,所有分享页面都带 noindex。用插件、MCP 或 Claude Code 插件发布时一律自动生成密码,所以链接被转发也没关系,没有密码就是一堵墙。(裸调 API 时你可以刻意省略密码,发一个公开页。)",
    faqQ2: "内容存在哪里?保留多久?", faqA2: "内容经 HTTPS 传输,存储在 Cloudflare 全球边缘(R2);密码只存单向哈希,不存明文。每条链接都会过期(这是刻意设计):匿名最长 7 天(一次性、不可续),登录最长 30 天且可在到期前续期;没有永久链接。你也可以随时手动删除,链接立即失效。",
    faqQ3: "发错了 / 不想给某人看了怎么办?", faqA3: "用「每人一链」时,撤销某一个人即时生效,其他人不受影响。否则:改共享密码可以挡住后续访问,或者直接删除页面,链接立即失效。有一点要说清——已经输过共享密码的读者凭会话 Cookie 最多还能读 24 小时,要立刻断掉就删除页面。",
    faqQ4: "免费吗?有什么限制?", faqA4: "核心能力现在全部免费——密码分享、合集、访问回执、每人一链、版本化,都不阉割。匿名即用、零注册:单文件 ≤ 512KB、最长 7 天、合集 ≤ 5 篇、每天 50 次。登录(免费)解锁更多——30 天可续链接、2MB、每人一链、版本历史、更大合集。禁止钓鱼与恶意内容,违规会被下架;核心能力永远免费、开源可自建。",
...
<body>
  ...
  <section class="band"><div class="wrap" style="text-align:center">
    ...
    <a class="try" href="https://${s.trySlug}.zhanjian.space" target="_blank" rel="noopener">
      <span class="lk">🔒</span>
      <span class="ta"><span class="tu">${s.trySlug}.zhanjian.space</span><span class="th">${s.tryTh}</span></span>
      <span class="go">→</span>
    </a>
  </div></section>
  ...
  <section><div class="wrap" style="text-align:center">
    ...
    <div class="vsc">
      <div class="vbar"><div class="dots"><i></i><i></i><i></i></div><span class="vt">${s.vsFile} — my-project</span></div>
      <div class="vbody">
        <div class="vside">
          <div class="vh">${s.vsRecent}</div>
          <div class="vi on"><span>🔒</span><span class="nm">${s.vsFile}</span><em>👁 12 · v2</em></div>
        ...
        <div class="vmain">
          <div class="vtabs"><span class="vtab">${s.vsFile}</span><span class="vcloud"><span class="ring"></span>☁️</span></div>
